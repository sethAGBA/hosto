...
class _AssurancesScreenState extends State<AssurancesScreen> with SingleTickerProviderStateMixin {
  ...
  Widget _buildActionsRow() {
    final canEdit = PermissionScope.of(context).canEdit('Assurances');
    final canView = PermissionScope.of(context).canView('Assurances');
    ...
    return Wrap(
      ...
      runSpacing: 10,
      children: [
        guard(_ActionButton(label: 'Nouvelle convention', icon: Icons.add_circle_outline, color: const Color(0xFF3B82F6)), canEdit),
        guard(_ActionButton(label: 'Verifier droits', icon: Icons.check_circle_outline, color: const Color(0xFF22C55E)), canEdit),
        guard(_ActionButton(label: 'Teletransmettre', icon: Icons.send_outlined, color: const Color(0xFFF59E0B)), canEdit),
